/*!
 * jQuery Roundrr Plugin (2012)
 *
 * This version written by Sterling Michel Extends Addy Osmani and extending Nirvana Tikku's RadMenu
 *
 */
...
(function ($, window) {
    ...
    var defaults = {
        ...
        onMouseOver: function ($selected) {
        }, //when the image is under the pick/viewer
        onMouseOut: function ($selected) {
        }, //when the image is under the pick/viewer
        onImageFocus: function ($selected) {
        }, //when the image is under the pick/viewer
        onImageBlur: function ($selected) {
        }, //when the image is not under the pick/viewer
        onNextBegin: function ($selected) {
        }, //called before the animation moves to next
        onPrevBegin: function ($selected) {
        }, //called before the animation moves to prev
        radius: 10, // in pixels
        animationEffect: 1,
        initialScale: 1,
    ...
    function init($menu, opts) {
        ...
        centerIcon = jQuery('.' + o.centerClass).wrapInner("<img class='centerImage' src='" + o.centerImageSrc + "'></img>").hide();
        centerIcon.fadeIn(2000);
        ...
        return $menu.each(function (m) {
            ...
            for (e in MENU) {
                $this.bind(e + roundrrwheel, $this, MENU[e]);
            }
        });
    }
    ...
    function selectMenuitem(evt) {
        ...
        if (!isInNested) {
            $this.parents("." + container.clz).roundrrwheel(index);
        } else {
            $this.roundrrwheel(index);
        }
        cancelBubble(evt);
    }
    ...
    /*
     * set the center accordingly
     * @param {type} $m
     * @returns {undefined}
     */
    function setCenterImage($m) {
        var reqLeftA = Math.floor($m.opts.requiredLeftA.replace('px', ''));
        var reqTopA = Math.floor($m.opts.requiredTopA.replace('px', ''));

        if ($.browser.chrome) {
            reqLeftA = reqLeftA - 116 + 'px';
            reqTopA = reqTopA + 10 + 'px';
        } else if ($.browser.mozilla) {
            reqLeftA = reqLeftA + 60 + 'px';
            reqTopA = reqTopA + 140 + 'px';
        }
        ...
        show: function (evt, fn) { // fn = user input onshow
            ...
            if (doAutoplay) {
                ...
                $(window).resize(function () {
                    ...
                    $m.opts.centerX = (width / 2) - 50;
                    $m.opts.centerY = (height / 2) - 45;
                    setCenterImage($m);
                });
    ...
    function switchItems($m, remove, add, posOffset, direction) {
        ...
        if (doAutoplay === false) {
            ...
            if (remove == add) {
                add = remove - 1; // ensure that we don't lose any items
            }
    ...
    function getCoords(idx, num, opts) {
        ...
        var angleOffset = opts.angleOffset * .013; // provide flexibility of angle
    ...
    function animateWheel($m, posOffset, direction) {
        ...
        $menuitems.each(function (i) {
            ...
            // Apply a mouse over action	
            $this.mouseover(function () {
                $m.opts.onMouseOver($m);
            });
