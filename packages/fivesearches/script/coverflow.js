...
(function ($) {
    ...
    var browserVersion = $.browser.version.replace(/^(\d+\.)(.*)$/, function () {
        return arguments[1] + arguments[2].replace(/\./g, '');
    });
    ...
    $.widget("ui.coverflow", {
        ...
        _create: function () {
            ...
            // overide until i can find how to properly center
            this.element.css(this.props[2], '110px');
        ...
        select: function (item, noPropagation) {
            ...
            this.element.stop().animate(animation, {
                ...
                easing: 'easeOutQuint',
                complete: function () {
                    $(this).find('li').eq(self.current).addClass('active');
                    //console.warn( $(this).find('li').eq(self.current) );
                }
            });
        ...
        _refresh: function (state, from, to) {
            ...
            this.items.each(function (i) {
                ...
                css[(($.browser.safari || $.browser.chrome) ? 'webkit' : 'Moz') + 'Transform'] = 'matrix(1,' + (mod * (side == 'right' ? -0.2 : 0.2)) + ',0,1,0,0) scale(' + (1 + ((1 - mod) * 0.3)) + ')';
