...
(function ($, undefined) {
  ...
  $.widget("simone.taskbar", {
    ...
    classes: {
      desktop: "simone-desktop",
      desktopPrefix: "simone-desktop-",
    ...
    _createDesktop: function () {
      var $taskbar = this.$elem;
      var appnames = this.options.applications;
      var applicationTotal = this.options.applicationTotal;
      var theclass = this.classes.desktopPrefix;

      for(var x=0; x<applicationTotal; x++) {
                // grab the name
        var appname = appnames[x];
  
        var $div = $('<div class="simone-desktop-icon" id="simone-desktop-' + appname.shortName.toLowerCase() +'">' +
                     ' <div class="simone-desktop-image">' + // data-appname="' + appname + '">' +
                     '  <img style="width: 40px; height: 40px;" src="/img/apps/' + appname.shortName.toLowerCase() + '_large.png" alt="eSearch"/>' +
                     ' </div>' +
                     ' <div class="simone-desktop-title">' + appname.shortName + '</div>' +
                     '</div>');
        
        // Add the icon to desktop
        $taskbar.parent().append($div);
        ...
        // Apply the double click
        $div.on('dblclick', function (evt) {
          evt.preventDefault();
          var desktop = $taskbar.data('desktop');
          var name = $taskbar.attr('id') + "-" + appname.appName;

          if(desktop.hasOwnProperty(name)) {
            console.info('App is currently in your taskbar');
            return;
          }

          // store the app to prevent duplicate call
          desktop[name] = x;
          $taskbar.data('desktop', desktop);
          ...
          // build the page
          $win.addClass('simone-menu-hidden fivesearchesWindow');
    ...
    _create: function () {
      ...
      //initialize the desktop queue
      this.$elem.data('desktop', {});
      ...
      $elems.each(function () {
        ...
        position.using = function( position, feedback ) {
          //console.warn("==", position);
              position.top = position.top - 20 + 'px';
              $( this ).css( position );
              $( "<div>" )
                .addClass( "arrow" )
...
(function ($, undefined) {
  ...
  $.widget("simone.window", $.ui.dialog, {
    ...
    destroy: function ( ) {
      ...
      // add the ability to delete if object is destroy
      var desktop = this.$taskbar.data('desktop');
      var name = this.$taskbar.attr('id') + "-" + this._cache.title;
      
      //console.warn("destroy:", name);
      if(desktop.hasOwnProperty(name)) {
        delete desktop[name];
        this.$taskbar.data('desktop', desktop);
      }
