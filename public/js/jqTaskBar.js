...
(function($){
    ...
    var jqTaskBarMethods = {
        init: function(options){
            return this.each(function(){
                ...
                $this.html('');
                $this.data('jqDesktopTaskBar', $.extend({},options, {settings:{_windows:[],_installApp:[]}}));                
            });
        },
        addWindow: function(jqWindow){
            return this.each(function(){
                ...
                jqWindow.each(function(){
                    ...
                    if ($.inArray(Window.attr('id'), $this.data('jqDesktopTaskBar').settings._installApp)>-1) return;
                    $this.data('jqDesktopTaskBar').settings._installApp.push(Window.attr('id'));
                    ...
                    var icon = $('<div class="jqdesktop-taskbar-icon" style="width:'+($.jqTaskBar.settings.iconWidth)+'px;height:' +  
				 ($.jqTaskBar.settings.iconHeight)+'px"><img src="'+(windowSettings.icon)+'" width="100%" height="100%"/></div>');
        ...
        _events: function(){
			...
			// Preview
	   		$this.jqTaskBar('setPreview', $this.data('jqWindow'));
                    },
                    function(){
                ...
                $this.on('click', function(){
                    ...
                    var status = Window.data('jqDesktopWindow').status; 
	...
	setPreview: function(jqWindow){
	    var $this = $(this);
            if (!jqWindow) return;
              var status = jqWindow.data('jqDesktopWindow').status;
	      if(status.minimized === true){
	        $this
